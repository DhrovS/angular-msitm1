...
@Component({
  selector: "app-s-update-admin",
  templateUrl: "./s-update-admin.component.html",
  styleUrls: ["./s-update-admin.component.css"],
  providers: [ ConnectingToDatabaseService, AlertService ]
})
export class SProdUpdateAdminComponent implements OnInit {
  SProdForm: FormGroup;
  loading = false;
  submitted = false;
  color;
  ...
  onSubmit() {
    ...
    this._dbService.patchSProd(this.route.snapshot.params.id,this.SProdForm.value)
    .pipe(first())
      .subscribe(
          data => {
              this.alertService.success('Registration successful', true);
              this.router.navigate(['/products/admin/1'])
